...
class DOMLMIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class DOMLMLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = DOMLMPredictionHeadTransform(config)
...
class DOMLMOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = DOMLMLMPredictionHead(config)
...
# Copied from transformers.models.bert.modeling_bert.BertLayer with Bert->DOMLM
class DOMLMLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = DOMLMAttention(config)
        self.is_decoder = config.is_decoder
...
class DOMLMModel(DOMLMPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = DOMLMEmbeddings(config)
        self.encoder = DOMLMEncoder(config)

        self.pooler = DOMLMPooler(config) if add_pooling_layer else None
    ...
    def forward(
        ...
        tag_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        ...
        ```python
        >>> from transformers import DOMLMProcessor, DOMLMModel
